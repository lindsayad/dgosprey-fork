...
/*
//Construct a default Sparse matrix as a 1x1 dense matrix
...
//Construct a sparse matrix (Use this constructor for temporary matrices)
SparseMatrix::SparseMatrix(int rows, int columns)
{
    if (rows <= 0 || columns <= 0)
    {
        mError(invalid_size);
        std::cout << "Defaulting to 1x1 matrix" << std::endl;
        rows = 1;
        columns = 1;
    }
  zero[0] = 0.0;
  ...
  num_rows = rows;
  num_cols = columns;
  Key.reserve(rows*columns);
}

//For setting values within the given sparcity pattern
double& SparseMatrix::operator()(int i, int j)
{
  ...
  {
    std::unordered_map<int, double>::iterator it = Key.find((i*num_cols)+j);
    if (it == Key.end())
    {
      zero[0] = 0.0;
      return zero[0];
...
//For accessing values within the given sparcity pattern
double SparseMatrix::operator()(int i, int j) const
{
	...
	{
    std::unordered_map<int, double>::const_iterator it = Key.find((i*num_cols)+j);
    if (it == Key.end())
    {
      return 0.0;
    }
...
//For Creating a Sparse Matrix from a dense Matrix M
SparseMatrix &SparseMatrix::operator=(const Matrix &M)
{
	if (this == &M)
	{
		return *this;
	...
	{
    num_rows=M.SparseMatrix::num_rows;
    num_cols=M.SparseMatrix::num_cols;
    zero[0] = 0.0;
    nz = 0;
		...
		for (int i=0; i<num_rows; i++)
		{
			for (int j=0; j<num_cols; j++)
			{
        this->edit(i, j, ( M.SparseMatrix::Data[(i*M.SparseMatrix::num_cols)+j] ) );
			}
		}
		return *this;
	}
...
: Matrix()
{
  ...
  num_cols = M.SparseMatrix::num_cols;
  num_rows = M.SparseMatrix::num_rows;
  Key.reserve(num_rows*num_cols);
  nz=0;
  std::unordered_map<int, double>::const_iterator it = Key.end();
	for (int i=0; i<num_rows; i++)
	{
		for (int j=0; j<num_cols; j++)
		{
      if (M.SparseMatrix::Data[(i*M.SparseMatrix::num_cols)+j] != 0.0)
      {
        Key.insert( it, { (i*num_cols)+j , M.SparseMatrix::Data[(i*M.SparseMatrix::num_cols)+j] } );
        nz++;
      }
      else
      {
...
SparseMatrix &SparseMatrix::operator=(const SparseMatrix &SM)
{
	...
	{
    nz=SM.SparseMatrix::nz;
    num_rows=SM.SparseMatrix::num_rows;
    num_cols=SM.SparseMatrix::num_cols;
    zero[0] = 0.0;
    Key = SM.SparseMatrix::Key;
		return *this;
	}
}
...
//Allocate size (This should only be called once after an initialization)
void SparseMatrix::set_size(int i, int j)
{
    if (i<=0 || j<= 0)
    {
        mError(invalid_size);
...
//Remove a element of the matrix from memory
void SparseMatrix::remove(int i, int j)
{
  ...
  //Check for existance of non-zero in this
  std::unordered_map<int, double>::const_iterator it = Key.find((i*num_cols)+j);
  if (it == Key.end())
  {
    //No Action
  ...
  {
    //If a non-zero exists, remove it
    Key.erase(it);
    nz--;
  }
}

//Function to delete all matrix data without reseting the size
void SparseMatrix::remove_all()
{
  zero[0] = 0.0;
  nz = 0;
  Key.clear();
...
//Add/Change an element to a sparse matrix
void SparseMatrix::edit(int i, int j, double value)
{
  if (i>=num_rows || j>=num_cols || i<0 || j<0)
  {
      mError(invalid_size);
  ...
  if (value == 0.0)
  {
    //If a non-zero exists, remove it
    this->remove(i,j);
    //Do not add in fill if the fill is a zero
    return;
  }
  else
  {
    ...
    if (it == Key.end())
    {
      Key.insert( it, { (i*num_cols)+j , value } );
      nz++;
    }
    else
    {
...
double SparseMatrix::norm()
{
  ...
  for(int i=0; i<num_rows; i++)
  {
    for (int j=0; j<num_cols; j++)
    {
      norm = norm + pow( fabs( this->operator()(i, j) ) ,2.0);
    }
  }
  norm = sqrt(norm);
  return norm;
...
//Calculates the inner product of two vectors
double SparseMatrix::inner_product(const Matrix& x)
{
  double prod = 0.0;
  if (x.SparseMatrix::num_cols != 1 || num_cols != 1)
  {
    mError(dim_mis_match);
    std::cout << "Inner Product can only be performed on vectors!" << std::endl;
    return prod;
  }
  else if (x.SparseMatrix::num_rows != num_rows)
  {
    mError(dim_mis_match);
    return prod;
  }
  ...
  {
    for (int i=0; i<x.SparseMatrix::num_rows; i++)
    {
      prod = prod + x(i,0)*this->operator()(i, 0);
    }
  }
  return prod;
}
...
//Calculates the inner product of two vectors
double SparseMatrix::inner_product(const SparseMatrix& sx)
{
  double prod = 0.0;
  if (sx.SparseMatrix::num_cols != 1 || num_cols != 1)
  {
    mError(dim_mis_match);
    std::cout << "Inner Product can only be performed on vectors!" << std::endl;
    return prod;
  }
  else if (sx.SparseMatrix::num_rows != num_rows)
  {
    mError(dim_mis_match);
    return prod;
  }
  ...
  {
    for (int i=0; i<sx.SparseMatrix::num_rows; i++)
    {
      prod = prod + sx(i,0)*this->operator()(i, 0);
    }
  }
  return prod;
}
...
SparseMatrix SparseMatrix::operator+(const Matrix &M)
{
    if(num_rows!=M.SparseMatrix::num_rows && num_cols!=M.SparseMatrix::num_cols)
    {
        mError(dim_mis_match);
        return *this;
    }
    ...
    {
      	SparseMatrix temp(num_rows, num_cols);
        for (int i=0; i<num_rows; i++)
        {
            for (int j=0; j<num_cols; j++)
            {
                temp.edit(i,j,this->SparseMatrix::operator()(i,j) + M.SparseMatrix::Data[(i*M.SparseMatrix::num_cols)+j]);
            }
        }
        return temp;
    }
...
SparseMatrix SparseMatrix::operator+(const SparseMatrix &SM)
{
    if(num_rows!=SM.SparseMatrix::num_rows && num_cols!=SM.SparseMatrix::num_cols)
    {
        mError(dim_mis_match);
        return *this;
    }
    else
    {
        ...
        for (int i=0; i<num_rows; i++)
        {
            for (int j=0; j<num_cols; j++)
            {
                temp.edit(i,j,this->SparseMatrix::operator()(i,j) + SM.SparseMatrix::operator()(i, j));
            }
        }
        return temp;
    }
...
SparseMatrix SparseMatrix::operator*(const double a)
{
    ...
    for (int i=0; i<num_rows; i++)
    {
        for (int j=0; j<num_cols; j++)
        {
            temp.edit(i,j,a * this->SparseMatrix::operator()(i,j));
        }
    }
    return temp;
}
...
SparseMatrix SparseMatrix::operator*(const Matrix &M)
{
    if (num_cols!=M.SparseMatrix::num_rows)
    {
      mError(dim_mis_match);
      return *this;
    }
    ...
    {
      SparseMatrix temp(num_rows, M.SparseMatrix::num_cols);
      double value = 0.0;
        
      int j=0;
      for(int i=0; i<num_rows; i++)
      {
        for (int J=0; J<M.SparseMatrix::num_cols; J++)
        {
          value = 0.0;
          j=0;
          for (int I=0; I<M.SparseMatrix::num_rows; I++)
          {
            value = value + (this->SparseMatrix::operator()(i, j) * M.SparseMatrix::Data[(I*M.SparseMatrix::num_cols)+J]);
              j++;
          }
          temp.edit(i,J,value );
        }
...
SparseMatrix SparseMatrix::operator*(const SparseMatrix &SM)
{
  if (num_cols!=SM.SparseMatrix::num_rows)
  {
    mError(dim_mis_match);
    return *this;
  }
  ...
  {
    SparseMatrix temp(num_rows, SM.SparseMatrix::num_cols);
    double value = 0.0;
    
    int j=0;
    for(int i=0; i<num_rows; i++)
    {
      for (int J=0; J<SM.SparseMatrix::num_cols; J++)
      {
        value = 0.0;
        j=0;
        for (int I=0; I<SM.SparseMatrix::num_rows; I++)
        {
          value = value+(this->SparseMatrix::operator()(i, j) * SM.SparseMatrix::operator()(I, J));
          j++;
        }
        temp.edit(i,J,value );
      }
...
//Transpose of a matrix
SparseMatrix &SparseMatrix::transpose(const Matrix &M)
{
	//A will be indexed by rows and cols
  this->set_size(M.SparseMatrix::num_cols, M.SparseMatrix::num_rows);
...
//Transpose of a matrix
SparseMatrix &SparseMatrix::transpose(const SparseMatrix &SM)
{
  if (this == &SM)
	{
    mError(dim_mis_match);
	...
	//A will be indexed by rows and cols
  this->set_size(SM.SparseMatrix::num_cols, SM.SparseMatrix::num_rows);
  
	for (int i=0; i<num_rows; i++)
	{
		for (int j=0; j<num_cols; j++)
		{
			this->edit(i,j,SM(j,i));
		}
	}
...
//Transposes the matrix MT then multiplies by matrix v in one step
SparseMatrix &SparseMatrix::transpose_multiply(const Matrix &MT, const Matrix &v)
{
  if (MT.SparseMatrix::num_rows != v.SparseMatrix::num_rows)
  {
    mError(dim_mis_match);
		return *this;
  } 
  ...
  {
    this->set_size(MT.SparseMatrix::num_cols, v.SparseMatrix::num_cols);
    double value = 0.0;
    int j=0;
		for(int i=0; i<num_rows; i++)
		{
			for (int J=0; J<v.SparseMatrix::num_cols; J++)
			{
        value = 0.0;
				j=0;
				for (int I=0; I<v.SparseMatrix::num_rows; I++)
				{
          value = value + (MT.SparseMatrix::Data[(j*MT.SparseMatrix::num_cols)+i] * v.SparseMatrix::Data[(I*v.SparseMatrix::num_cols)+J]);
					j++;
				}
        this->edit(i,J,value);
			}
...
//Transposes the matrix MT then multiplies by matrix v in one step
SparseMatrix &SparseMatrix::transpose_multiply(const SparseMatrix &MT, const Matrix &v)
{
  if (MT.SparseMatrix::num_rows != v.SparseMatrix::num_rows)
  {
    mError(dim_mis_match);
  ...
  else
  {
    ...
    int j=0;
        ...
        value = 0.0;
				...
				for (int I=0; I<v.SparseMatrix::num_rows; I++)
				{
          			value = value + (MT.SparseMatrix::operator()(j, i) * v.SparseMatrix::Data[(I*v.SparseMatrix::num_cols)+J]);
					j++;
				}
        	this->edit(i,J,value);
			}
...
//Transposes the matrix MT then multiplies by matrix v in one step
SparseMatrix &SparseMatrix::transpose_multiply(const Matrix &MT, const SparseMatrix &v)
{
  if (MT.SparseMatrix::num_rows != v.SparseMatrix::num_rows)
  {
    mError(dim_mis_match);
    ...
    int j=0;
		for(int i=0; i<num_rows; i++)
		{
			for (int J=0; J<v.SparseMatrix::num_cols; J++)
			{
				...
				for (int I=0; I<v.SparseMatrix::num_rows; I++)
				{
          			value = value + (MT.SparseMatrix::Data[(j*MT.SparseMatrix::num_cols)+i] * v.SparseMatrix::operator()(I, J));
					j++;
				}
				this->edit(i,J,value);
			}
...
//Transposes the matrix MT then multiplies by matrix v in one step
SparseMatrix &SparseMatrix::transpose_multiply(const SparseMatrix &MT, const SparseMatrix &v)
{
  if (MT.SparseMatrix::num_rows != v.SparseMatrix::num_rows)
  {
		mError(dim_mis_match);
    ...
    int j=0;
		for(int i=0; i<num_rows; i++)
		{
			for (int J=0; J<v.SparseMatrix::num_cols; J++)
			{
				...
				for (int I=0; I<v.SparseMatrix::num_rows; I++)
				{
          			value = value + (MT.SparseMatrix::operator()(j, i) * v.SparseMatrix::operator()(I, J));
					j++;
				}
        		this->edit(i,J,value);
			}
...
//Display SparseMatrix to console
void SparseMatrix::Display(const std::string Name)
{
	std::cout << Name << " = " << std::endl;
	if (num_rows == 0 || num_cols == 0)
	{
...
//Function to solve the tridiagonal matrix using the My Algorithm (works for any tridiagonal matrix)
SparseMatrix &SparseMatrix::ladshawSolve(const Matrix& A, const Matrix& d)
{
	//Solves the system Ax=b when A is a tridiagonal matrix
	this->set_size(d.SparseMatrix::num_rows, d.SparseMatrix::num_cols);
  
	//Check dimensions of A and b to ensure they will work out
	if (num_rows!=A.SparseMatrix::num_cols || num_cols>1)
	{
		mError(dim_mis_match);
		return *this;
	}
	...
	for (int i=0; i<num_rows; i++)
	{
		if (i==0)
			...
			this->edit(i,0,dpp[i] - (app[i] * this->operator()(i-1, 0)));
	}
	cp.clear();
	dp.clear();
	ap.clear();
...
//Function to solve the tridiagonal matrix using the My Algorithm (works for any tridiagonal matrix)
SparseMatrix &SparseMatrix::ladshawSolve(const SparseMatrix& A, const Matrix& d)
{
  if (this == &A)
	{
    	mError(arg_matrix_same);
...
//Function to solve the tridiagonal matrix using the My Algorithm (works for any tridiagonal matrix)
SparseMatrix &SparseMatrix::ladshawSolve(const Matrix& A, const SparseMatrix& d)
{
  if (this == &d)
	{
    	mError(arg_matrix_same);
...
//Function to solve the tridiagonal matrix using the My Algorithm (works for any tridiagonal matrix)
SparseMatrix &SparseMatrix::ladshawSolve(const SparseMatrix& A, const SparseMatrix& d)
{
  if (this == &A || this == &d)
	{
   	 	mError(arg_matrix_same);
...
//Function to fill in a tridiagonal matrix with constant coefficients
SparseMatrix &SparseMatrix::tridiagonalFill(const double A, const double B, const double C, bool Spherical)
{
	//Check for square matrix
	if (num_cols!=num_rows)
	{
...
//Fill in the Constant IC for each system
SparseMatrix &SparseMatrix::ConstantICFill(const double IC)
{
	if (num_cols>1)
	{
		mError(dim_mis_match);
...
//Function to fill in the tridiagonal matrix for 1-D Conservation Laws
SparseMatrix &SparseMatrix::tridiagonalVectorFill(const std::vector<double> &A, const std::vector<double> &B, const std::vector<double> &C)
{
	//Check for square matrix
	if (num_cols!=num_rows)
	{
...
//Function to fill in a column matrix based on a vector of values
SparseMatrix &SparseMatrix::columnVectorFill(const std::vector<double> &A)
{
	if (num_cols>1)
	{
		mError(dim_mis_match);
...
//Perform a projection of the n+1 time level from previous stored information
SparseMatrix &SparseMatrix::columnProjection(const Matrix& b, const Matrix& b_old, const double dt, const double dt_old)
{
	if (b.SparseMatrix::num_cols>1 || b_old.SparseMatrix::num_cols>1 || b.SparseMatrix::num_rows!=b_old.SparseMatrix::num_rows)
	{
		mError(dim_mis_match);
		return *this;
	}
  if (this == &b || this == &b_old)
	...
	this->set_size(b.SparseMatrix::num_rows,b.SparseMatrix::num_cols);
	double dtm1 = dt_old;
	if (dtm1 == 0.0) dtm1 = dt;
	for (int i=0; i<num_rows; i++)
	{
		this->edit(i,0, b(i,0) + ((dt/(2.0*dtm1)) * (b(i,0) - b_old(i,0))) );
	}
	return *this;
}

//Perform a projection of the n+1 time level from previous stored information
SparseMatrix &SparseMatrix::columnProjection(const SparseMatrix& b, const Matrix& b_old, const double dt, const double dt_old)
{
	if (b.SparseMatrix::num_cols>1 || b_old.SparseMatrix::num_cols>1 || b.SparseMatrix::num_rows!=b_old.SparseMatrix::num_rows)
	{
		mError(dim_mis_match);
...
//Perform a projection of the n+1 time level from previous stored information
SparseMatrix &SparseMatrix::columnProjection(const Matrix& b, const SparseMatrix& b_old, const double dt, const double dt_old)
{
	if (b.SparseMatrix::num_cols>1 || b_old.SparseMatrix::num_cols>1 || b.SparseMatrix::num_rows!=b_old.SparseMatrix::num_rows)
	{
		mError(dim_mis_match);
...
//Perform a projection of the n+1 time level from previous stored information
SparseMatrix &SparseMatrix::columnProjection(const SparseMatrix& b, const SparseMatrix& b_old, const double dt, const double dt_old)
{
	if (b.SparseMatrix::num_cols>1 || b_old.SparseMatrix::num_cols>1 || b.SparseMatrix::num_rows!=b_old.SparseMatrix::num_rows)
	{
		mError(dim_mis_match);
...
//Fill in the Dirichlet BC for non-spherical coordinates
SparseMatrix &SparseMatrix::dirichletBCFill(int node, const double coeff, double variable)
{
	if (num_cols>1)
	{
		mError(dim_mis_match);
	...
	{
    this->remove_all();
    this->edit(node,0,coeff*variable);
		return *this;
	}
}
*/
